...
function Menu() {    
    ...
    return (
        <div className="w-full lg:static fixed bottom-0 left-0 right-0 mx-auto flex justify-center items-center z-[103] lg:bg-transparent bg-bgcard py-3 gap-5 lg:mx-5 ">
            {items.map(menu => <ul className="flex lg:flex-row flex-col items-center" key={menu.id}>
                ...
                <li><Link className={`text-pink-500 hover:text-secondary flex justify-center items-center w-20 z-0`} to={menu.link}>{menu.name}</Link></li>
            </ul>)}
        </div>
    )
}
