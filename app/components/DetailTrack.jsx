...
function DetailTrack() {
    const { isPlaying,isPlayerVisible,albumDetail } = useSelector((state) => state.songs);

    const dispatch = useDispatch()
    const words = albumDetail?.title?.split(" ") || [];
    ...
    return (
        <div className="flex flex-col ">
            <div className="flex gap-10 mt-36 " style={{ display: isPlayerVisible ? 'none' : undefined }}>
                <img src={albumDetail?.cover_url} className="md:size-72 size-44 rounded-xl shadow-black shadow-bottom object-cover" alt="" />
                <div className="flex flex-col justify-between gap-10">
                    ...
                    <span className="text-primarytxt text-sm md:flex hidden">{albumDetail?.description}</span>
            ...
            <ListMusicOfAlbum tracks={albumDetail?.tracks} />
        </div>
    );
}
